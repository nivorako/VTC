...
const stripePromise = loadStripe(
    "pk_test_51RZw5JRkTF6EdgwvRqC30aXrGs7GbAJ4WzHJUMgTR6WynTG6xjKI6KdoDXv52F4HpmcahdRCFIdQkPZf9ZE6cCsH00qKvcqxfe"
);
...
const PaymentForm = () => {
    ...
    if (!locationState) {
        return (
            <FormContainer>
                ...
                <p>
                    Pour effectuer un paiement, veuillez d'abord sélectionner un
                    trajet.
                </p>
                <Button
                    type="button"
    ...
    const checkPaymentStatus = async (paymentIntentId: string) => {
        try {
            ...
            setError(
                `Erreur lors de la vérification du statut: ${err instanceof Error ? err.message : "Erreur inconnue"}`
            );
            setLoading(false);
        }
    };
        ...
        try {
            ...
            try {
                ...
                if (!contentType || !contentType.includes("application/json")) {
                    ...
                    throw new Error(
                        `Réponse non-JSON reçue du serveur: ${textResponse.substring(0, 100)}...`
                    );
                }
                ...
                if (stripeError) {
                    ...
                    if (stripeError.type === "card_error") {
                        throw new Error(
                            `Erreur de carte: ${stripeError.message}`
                        );
                    } else if (stripeError.type === "validation_error") {
                        throw new Error(
                            `Erreur de validation: ${stripeError.message}`
                        );
                    } else {
                        throw new Error(
                            `Erreur inattendue: ${stripeError.message}`
                        );
                    }
                }
            ...
            setError(
                `Erreur: ${err instanceof Error ? err.message : "Une erreur inconnue est survenue"}`
            );
            setLoading(false);
        }
    };
            ...
            {success ? (
                ...
                <>
                    ...
                    <TestCardsInfo>
                        ...
                        <ul>
                            <li>
                                <strong>Carte qui réussit:</strong> 4242 4242
                                4242 4242
                            </li>
                            <li>
                                <strong>Carte qui échoue:</strong> 4000 0000
                                0000 0002
                            </li>
                        </ul>
                        <p>
                            Utilisez n'importe quelle date future, n'importe
