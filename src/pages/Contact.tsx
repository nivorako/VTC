...
const schema = yup.object().shape({
  clientType: yup.string().required('Veuillez sélectionner un type de client'),
  name: yup.string().required('Le nom est requis'),
  email: yup.string().email('Format d\'email invalide').required('L\'email est requis'),
  phone: yup.string().required('Le téléphone est requis'),
  requestType: yup.array(yup.string().required()).min(1, 'Veuillez sélectionner au moins un type de demande').required(),
		...
		<ContactSection>
			...
			<FormContainer onSubmit={handleSubmit(onSubmit)}>
				<FormGroup>
					...
					<CheckboxGroup>
						<CheckboxLabel>
						<input type="radio" value="privé" {...register('clientType')} />
						Privé
						</CheckboxLabel>
						<CheckboxLabel>
						<input type="radio" value="particulier" {...register('clientType')} />
						Particulier
						</CheckboxLabel>
					</CheckboxGroup>
					{errors.clientType && <ErrorMessage>{errors.clientType.message}</ErrorMessage>}
				</FormGroup>
				...
				<FormGroup>
					...
					<CheckboxGroup>
						<CheckboxLabel>
							<input type="checkbox" value="Longue distance" {...register('requestType')} />
							Longue distance
						</CheckboxLabel>
						<CheckboxLabel>
							<input type="checkbox" value="Rapatriement" {...register('requestType')} />
						Rapatriement
						</CheckboxLabel>
						<CheckboxLabel>
							<input type="checkbox" value="Évènement" {...register('requestType')} />
							Évènement
						</CheckboxLabel>
						<CheckboxLabel>
							<input type="checkbox" value="Transfert" {...register('requestType')} />
							Transfert gare/aéroport
						</CheckboxLabel>
					</CheckboxGroup>
					{errors.requestType && <ErrorMessage>{errors.requestType.message}</ErrorMessage>}
				</FormGroup>
